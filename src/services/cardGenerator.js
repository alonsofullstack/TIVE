...
const TIVE_COMPLETO_FIELDS = [
    ...
    { key: 'ruedas', dataKey: 'ruedas', x: 104.1, y: 81.8, dx: -7, dy: -4, size: 8, bold: false },
    { key: 'ejes', dataKey: 'ejes', x: 104.1, y: 67, dx: -7, dy: -3, size: 8, bold: false },
...
module.exports = function (bot) {
    ...
    async function finalizarInsercionQR(chatId, buffer, placa, hash, messageId = null) {
        ...
        const posX = (QR_X_ORIGINAL / 100) * width;
    ...
    async function iniciarCapturaFaltantesFisicaPvcCompletar(chatId, datos, sourceBuffer = null) {
        ...
        if (missingFields.length === 0) {
            ...
            await generarTIVE(chatId, prepared, null, sourceBuffer, { anv: 'TARJETA FISICA ADELANTE.pdf', rev: 'TARJETA FISICA ATRAS.pdf' }, {
                noQR: true,
                cropTopAnv: 35, cropBottomAnv: 35, cropLeftAnv: 1, cropRightAnv: 1,  // recorte ANVERSO
